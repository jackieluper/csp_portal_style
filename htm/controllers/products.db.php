...
//Grabbing the product information to display in top offers
    $resultOffer1 = $conn->query("select offer.display_name, list_price from offer, offer_price where offer.id='$hotOffers[$i]' and offer_id='$hotOffers[$i]'");
...
$result1 = $conn->query("select offer.id, offer.display_name, offer.license_agreement_type, offer.purchase_unit, offer.sku, offer_price.list_price from offer, offer_price where offer.id=offer_price.id and offer.license_agreement_type='$entity'");
$index = 0;
